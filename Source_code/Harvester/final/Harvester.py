...
fremantlevsrichmond1 = []            
for x in combos(fremantlevsrichmond,1000000): 
    fremantlevsrichmond1.append(x)
...
while tweetCount < maxTweets: 
    ...
    for word in fremantlevsrichmond1:
        searchQuery = word
        print word
        hcount =0
        recent=""
        ...
        try:
            ...
            for tweet in new_tweets:
                try:
                    ...
                    if(("Jun"  in date)):
                        ...
                        source_v = (json_val["source"]).encode('utf-8').strip()
                        source_val = str(source_v)
                        if("andr" in source_val.lower()):
                            androidc = androidc +1
                        elif("web" in source_val.lower()):
                            webc = webc +1
                        elif("deck" in source_val.lower()):
                            tdc = tdc+1
                        elif("iph" in source_val.lower()):
                            iphone = iphone +1      
                        elif("ios" in source_val.lower()):
                            ios = ios +1    
                        else:
                            otherd = otherd +1
                        ...
                        if((word.find("fremantle")>=0) or (word.find("freo")>=0)or (word.find("docker")>=0)):
                            if((text.find("fremantle")>=0) or (text.find("freo")>=0)or (text.find("docker")>=0)):
                                if(rtcount>maxrt1):
                                    maxrt1 = rtcount1
                                    mostrt1 = _id
                                ...
                                if(text.find("lead")>=0):
                                    winteam1 = winteam1+1        
                        if((word.find("richmond")>=0) or (word.find("tiger")>=0))    :
                            if((text.find("richmond")>=0) or (text.find("tiger")>=0) )    :
                                if(rtcount>maxrt2):
                                    maxrt2 = rtcount2
                                    mostrt2 = _id
                        ...
                        #db.save(doc)
                        tweetData = open('fremantlevsrichmondTweets.csv','a')
            ...
            if(tweetCount>2000):
                ...
                tweetstats = open('STATS_round10.csv','a')
                ...
                if((rtcount1-savert1 > 200) or (rtcount2-savert2 >200) or (fcount1-savefc1 >200) or (fcount2-savefc2 > 200) or (winteam1-savewt1 >30) or (winteam2-savewt2 >30) or saveft != str(ftag) or savefl != str(flist) or savemr1 != mostrt1 or savemr2 != mostrt2 or savefp != str(fplist)):
                    ...
                    tweetstats.write('fremantle vs richmond')
                    ...
                    tweetstats.write('\n')
                    stats5 = "mostrt1:::"+str(mostrt1)+"  mostrt2:::"+str(mostrt2)+" iphone::::"+str(iphone)+" ios:::"+str(ios)+" web client:::"+str(webc)+" android:::"+str(androidc)+" tweetdeck:::"+str(tdc)+" other dev:::"+ str(otherd)
